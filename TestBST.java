...
public class TestBST {
	...
	public static void main(String[] args) {
		...
		try {
			...
			if (res != null) System.out.println("Test 12 failed");
			    ...
			    res = tree.get(tree.getRoot(),new Position(1,0));
			    if (res == null) System.out.println("Test 12 failed");
			    else {
				res = tree.get(tree.getRoot(),new Position(19,10));
				if (res == null) System.out.println("Test 12 failed");
		...
		try {
			...
			if (res.getPosition().compareTo(new Position(2,25)) == 0)
		...
		// Test remove method
		p = new Position(39,19);
		try {
			tree.remove(tree.getRoot(),p);
			res = tree.successor(tree.getRoot(),new Position(39,19));

			if (res.getPosition().compareTo(new Position(0,20)) != 0)
				System.out.println("Test 16 failed");
			else {
			    res = tree.get(tree.getRoot(),new Position(39,4));
	...
	private static void insertPoints(BufferedImage im, int xf, int yf, int xl, 
		...
		if ((yf < yl) || ((yf == yl) && (xf <= xl))) {
			...
			tree.put(tree.getRoot(),new Pixel(new Position(xm,ym),rgb));
